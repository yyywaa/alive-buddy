...
静默消息会被写入记忆上下文，但不会触发底层 ReAct 引擎和 LLM 消耗。
...
- **L1 感知层**：最近 N 条对话，SQLite 存储，直接注入上下文。超出容量（默认 20 条）后自动将最旧消息异步总结进 L2
- **L2 事件层**：LLM 对过期对话生成的"剧情梗概"，SQLite 存储。对话空闲超阈值（默认 2 小时）也会触发段落总结
- **L3 印象层**：从 L2 提炼的绝对事实（"用户喜欢猫"），ChromaDB 存储，按语义检索后注入上下文。睡眠时段（22:00~08:00）自动批量固化 L2 → L3

以上阈值均可通过 `CharacterConfig.memory` 配置。多模态降级：上下文中最多保留最后 3 张图片，更早的图片会被替换为其文字描述。

---

## 当前状态

核心链路（被动 reAct 回复 + 主动脉搏决策 + 三层记忆闭环）已端到端打通并通过真实 LLM 验证；运行状态持久化到 SQLite，重启后自动恢复。`npm test` 提供 28 个单元/集成用例，CI 见 `.github/workflows/ci.yml`。Skills 与用户反馈微调仍在开发中。详见 [docs/PLAN.md](docs/PLAN.md)。
